run_simulation <- function(neutral_loci=3, pop1_range_limit_left=0, pop1_range_limit_right=0.48, pop2_range_limit_left=0.52,
                           ...
                           pref_ratio=0.55, max_generations=10, per_reject_cost=0, meandispersal=0.01, growth_rate=1.05,
                           beginning_columns=1, survival_fitness_method=1, beta=1, male_trait_loci=1, assort_mating=1, 
                           female_trait_loci=1, underdominant_loci=3, same_loci_MTL_UDL=TRUE,
                           mating_trait_dominance=1, dom_coefficient=0.55, hybrid_fitness=1){
  ...
  for (time in 1:max_generations) {
    ...
    incProgress(1/max_generations)}
  #}
  })
  return(pop_matrix)
}

#test <- run_simulation(max_generations = 20)
...
plot_freqs <-  function(format_output, fit_neutral=FALSE, fit_trait=FALSE){
  ...
  # base plot
  p <- ggplot(format_output, aes(x=location, y=freqs, color=freqs)) +
